...
static void multi_send_packet(int playerNumber, void *packet, BYTE dwSize)
{
	TPkt pkt;
	...
	if (!SNetSendMessage(playerNumber, &pkt.hdr, pkt.hdr.wLen))
		nthread_terminate_game("SNetSendMessage0");
}

void NetSendLoPri(int playerNumber, BYTE *pbMsg, BYTE bLen)
{
	if (pbMsg && bLen) {
		multi_copy_packet(&sgLoPriBuf, pbMsg, bLen);
		multi_send_packet(playerNumber, pbMsg, bLen);
	}
}

void NetSendHiPri(int playerNumber, BYTE *pbMsg, BYTE bLen)
{
	BYTE *hipri_body;
	BYTE *lowpri_body;
	DWORD size, len;
