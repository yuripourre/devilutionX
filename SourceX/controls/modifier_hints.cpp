...
{
	if (hint.is_dpad)
		return Controller::IsControllerButtonPressed(ControllerButton_BUTTON_DPAD_UP);
	return Controller::IsControllerButtonPressed(ControllerButton_BUTTON_Y);
}

bool IsRightActive(const CircleMenuHint &hint)
{
	if (hint.is_dpad)
		return Controller::IsControllerButtonPressed(ControllerButton_BUTTON_DPAD_RIGHT);
	return Controller::IsControllerButtonPressed(ControllerButton_BUTTON_B);
}

bool IsBottomActive(const CircleMenuHint &hint)
{
	if (hint.is_dpad)
		return Controller::IsControllerButtonPressed(ControllerButton_BUTTON_DPAD_DOWN);
	return Controller::IsControllerButtonPressed(ControllerButton_BUTTON_A);
}

bool IsLeftActive(const CircleMenuHint &hint)
{
	if (hint.is_dpad)
		return Controller::IsControllerButtonPressed(ControllerButton_BUTTON_DPAD_LEFT);
	return Controller::IsControllerButtonPressed(ControllerButton_BUTTON_X);
}
